...
Zur Lösung der Aufgabe können randomisierte Felder erzeugt werden, wie es der Test-Code in Listing~\ref{lst:test} demonstriert. Dann wird überprüft, in welchen Bereichen sich $c4$ bewegt. Da jeder auf Vergleichsoperationen basierende Sortieralgorithmus zu der Laufzeitklasse $\Omega(n\ log\ n)$ gehört\footnote{\cite[154]{OW17b}, Skript (Teil2) S. 180}, kann $c4$ ignoriert werden, wenn $c4 < n\ log\ n$ gilt.
...
Uns interessieren vor allem die folgenden Intervalle$\footnote{siehe dazu auch Tabelle~\ref{tab:calls}}:
...
Die Ausführung der Tests zeigen, dass sich die Aufrufanzahl von $c4$ bei großen $n$ unterhalb $n^{1.3}$ bewegt, im worst-case zwischen $n^{1.3}$ und $n^2$.
...
\begin{lstlisting}[language=java,label=lst:shellsort,caption={Alternative Implementierung von Shellsort.}]
...
\begin{table} %[hbtp]
    \begin{center}
        \begin{tabular}{|c |c |c |c | c| c| c}
            ...
            \hline
        \end{tabular}
        \caption{Die Aufrufzahlen für $c2$ für verschiedene $n$. Schlüsselverlgeiche spielen an dieser Stelle keine Rolle. Grün hinterlegte Werte sind kleiner als $c2$, rote sind größer. Mit $n > 2156$ wächst $O(n^\frac{4}{3})$ schneller als $c2$. $O(n\ log\ n)$ wächst für $n > 3865$ langsamer als $O(n^\frac{4}{3})$ (gelb), aber immer noch schneller als $c2.
        }
        \label{tab:calls}
    \end{center}
\end{table}
