...
\begin{table} %[hbtp]
    \begin{center}
        \begin{tabular}{|c |c |c |c | c| c| c}
            ...
            \hline
        \end{tabular}
        \caption{Die Aufrufzahlen für $c2$ für verschiedene $n$. Schlüsselvergleiche spielen an dieser Stelle keine Rolle. Grün hinterlegte Werte sind kleiner als $c2$, rote sind größer. Mit $n > 2156$ wächst $O(n^\frac{4}{3})$ schneller als $c2$. $O(n\ log\ n)$ wächst für $n > 3865$ langsamer als $O(n^\frac{4}{3})$ (gelb).
        }
        \label{tab:calls}
    \end{center}
\end{table}
