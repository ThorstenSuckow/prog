...
\begin{lstlisting}[language=java,label=lst:counter]
...
Zur Lösung der Aufgabe genügt es jetzt, randomisierte Felder zu erzeugen, wie es der Test-Code in Listing~\ref{lst:test} demonstriert. Nun wird überprüft, in welchen Bereichen sich $c4$ bewegt. Da bereits bekannt ist, das jeder auf Vergleichsoperationen basierende Sortieralgorithmus zu der Laufzeitklasse $\Omega(n\ log\ n)$ gehört, kann $c4$ ignoriert werden, wenn $c4 < n\ log\ n$ gilt.
Allerdings wird $n^1.3$ ab $n=3865$ schneller wachsen als $n\ log\ n$ (siehe Abbildung~\ref{fig:lognplot}), weshalb das in der Auswertung berücksichtigt werden muss - man könnte sonst fälschlicherweise meinen, das in dem geg. Fall die Laufzeit im Durchschnitt auch $n\ log \n$ beträgt.
...
\begin{figure}[h]
    ...
    \caption{Ab $n=3865$ wächst $x^1.3$ schneller als $n\ log \n$.}
\label{fig:bestcase}
\end{figure}

die Ausgabe der Anzahl der   als $n\ log\ n$, weshalb wird noch überprüfen allerdings wächst   Uns interessieren vor allem die folgenden Intervalle$\footnote{ab einem bestimmten $n$ wird $n\ log\ n$ kleiner als $n^1.3$, siehe dazu Tabelle~\ref{tab:calls}}:

\begin{itemize}
     \item $n^{1.3} \leq x \leq n\ log \n$
     \item $n\ log \n \leq x \leq n^{1.3}$
     \item $n\ log \n \leq n^{1.3} \leq x \leq n^2$

\end{itemize}

\begin{lstlisting}[language=java,label=lst:test]
...
\begin{table} %[hbtp]
    \begin{center}
        \begin{tabular}{|c |c |c |c | c| c| c}
            ...
            \hline
            $1.                                        000.000$ & $18.000.007$ & $3.981.071,70$ & $6.30..E7$ & $1.99..E7$ & $1.0E12$ \\
            \hline
        \end{tabular}
        \caption{Die Aufrufzahlen für $c2$ für verschiedene $n$ mit randomisierten Schlüsseln. Grün hinterlegte Werte sind kleiner als $c2$, rote sind größer. Mit $n > 2156$ wächst $O(n^\frac{4}{3})$ schneller als $c2$. $O(n\ log\ n)$ wächst ab einem bestimmten Wert ebenfalls langsamer als $O(n^\frac{4}{3})$ (gelb) aber immer noch schneller als $c2$, ohne Beweis vermuten wir $T_{c2}(n) = O(n\ log\ n)$.
        }
        \label{tab:calls}
    \end{center}
\end{table}
